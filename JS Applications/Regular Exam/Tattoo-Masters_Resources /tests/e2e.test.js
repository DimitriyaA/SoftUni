...
const mockData = {
    ...
    "catalog": [
    {
     ...
     "_ownerId": "0001",
      "type": "The Mad Bunny",
      "imageUrl": "/images/mad bunny.png",
      "description": "A quirky and edgy tattoo featuring an animated bunny with a mischievous grin, clutching a knife, blending cuteness with a dark twist.",
      "userType": "Tattoo Artist",
    ...
    },
    {
      "_id": "1002",
      "_ownerId": "0002",
      "type": "Japanese Dragon",
      "imageUrl": "/images/japanese dragon.png",
      "description": "A majestic Japanese dragon wrapped in swirling clouds and flames, symbolizing strength, wisdom, and protection.",
      "userType": "Tattoo Enthusiast",
...
describe("E2E tests", function () {
    ...
    describe("Navigation bar [ 10 Points ]", () => {
        it("Logged user should see correct navigation [ 2.5 Points ]", async function () {
            ...
            expect(await page.isVisible("nav >> text=Add Tattoo")).to.be.true;
        ...
        it("Guest user should see correct navigation [ 2.5 Points ]", async function () {
            ...
            expect(await page.isVisible("nav >> text=Add Tattoo"), "Create is visible").to.be.false;
        ...
        it("Guest user navigation should work [ 2.5 Points ]", async function () {
            ...
            let tattoosBtn = await page.waitForSelector('nav >> text=Collection', { timeout: interval });
            await tattoosBtn.click();

            await page.waitForSelector('#tattoos', { timeout: interval });
        ...
        it("Logged in user navigation should work [ 2.5 Points ]", async function () {
            ...
            await page.waitForSelector('#tattoos', { timeout: interval });
            let createBtn = await page.waitForSelector('nav >> text=Add Tattoo', { timeout: interval });
    ...
    describe("Home Page [ 5 Points ]", function () {
        it("Show Home page text [ 2.5 Points ]", async function () {
            await page.goto(host);
            await page.waitForSelector('text=Whether you\'re a seasoned artist, a collector of ink, or someone looking for inspiration for their first tattoo,', { timeout: interval });
            expect(await page.isVisible("text=Whether you\'re a seasoned artist, a collector of ink, or someone looking for inspiration for their first tattoo,")).to.be.true;
        });

        it("Show Home page button and navigate to register [ 2.5 Points ]", async function () {
            await page.goto(host);
            await page.waitForSelector('#join-us', { timeout: interval });
            await page.click('#join-us');
    ...
    describe("Dashboard Page [ 15 Points ]", function () {
        ...
        it("Check Collection page with 0 tattoos [ 2.5 Points ]", async function () {
            const { get } = await handle(endpoints.catalog);
            get([]);
            ...
            await page.waitForSelector('text=Collection is empty, be the first to contribute', { timeout: interval });
            expect(await page.isVisible('text=Collection is empty, be the first to contribute')).to.be.true;
        ...
        it("Check Collection have correct images [ 2.5 Points ]", async function () {
            ...
            await page.waitForSelector(".tattoo img", { timeout: interval });
            const images = await page.$$eval(".tattoo img", (t) =>
                t.map((s) => s.src)
            );
        ...
        it("Check Collection have correct Type [ 2.5 Points ]", async function () {
            const { get } = await handle(endpoints.catalog);
            get(mockData.catalog);
            const data = mockData.catalog;
            ...
            await page.waitForSelector(".tattoo .type", { timeout: interval });
            const categories = await page.$$eval(".tattoo .type", (t) =>
            ...
            expect(categories[2]).to.contains(`${data[2].type}`);
        });

        it("Check Collection have correct User Type [ 2.5 Points ]", async function () {
            const { get } = await handle(endpoints.catalog);
            ...
            await page.waitForSelector(".tattoo .user-type", { timeout: interval });
            const categories = await page.$$eval(".tattoo .user-type", (t) =>
                t.map((s) => s.textContent)
            );

            expect(categories.length).to.equal(2);
            expect(categories[0]).to.contains(`${data[0].userType}`);
            expect(categories[1]).to.contains(`${data[1].userType}`);
        });

        it("Check Collection have Learn More button [ 2.5 Points ]", async function () {
            const { get } = await handle(endpoints.catalog);
            get(mockData.catalog.slice(0, 2));
            const data = mockData.catalog.slice(0, 2);
            ...
            await page.waitForSelector('.tattoo >> text=Learn More', { timeout: interval });
            const buttons = await page.$$eval(".tattoo >> text=Learn More", (t) =>
                t.map((s) => s.textContent)
            );
    ...
    describe("CRUD [ 50 Points ]", () => {
        describe('Create [ 12.5 Points ]', function () {
            it("Create does NOT work with empty fields [ 2.5 Points ]", async function () {
                ...
                let addCharacterBtn = await page.waitForSelector('text=Add Tattoo', { timeout: interval });
            ...
            it("Create makes correct API call [ 2.5 Points ]", async function () {
                ...
                let additionalInfoElement = await page.waitForSelector('[name="user-type"]', { timeout: interval });
                let submitBtn = await page.waitForSelector('[type="submit"]', { timeout: interval });

                await typeElement.fill(data.type);
                await imageElement.fill(data.imageUrl);
                await descriptionElement.fill(data.description);
                await additionalInfoElement.selectOption(data.userType);
            ...
            it("Create sends correct data [ 2.5 Points ]", async function () {
                ...
                expect(postData.userType).to.equal(data.userType);
            });
        ...
        describe('Details [ 10 Points ]', function () {
            it("Details calls the correct API [ 2.5 Points ]", async function () {
                ...
                let learnMoreButton = await page.waitForSelector(`.tattoo-info:has-text("${data.type}") >> .details-btn`, { timeout: interval });
            ...
            it("Details with guest calls shows correct info [ 2.5 Points ]", async function () {
                ...
                let learnMoreElement = await page.waitForSelector('#user-type', { timeout: interval });
                ...
                expect(learnMore).to.contains(data.userType);
        ...
        describe('Edit [ 17.5 Points ]', function () {
            ...
            it("Edit should populate form with correct data [ 2.5 Points ]", async function () {
                ...
                await page.waitForSelector('.edit-form select', { timeout: interval });
                ...
                const selects = await page.$$eval(".edit-form select", (s) => s.map((s) => s.value));
            
                expect(inputs[0]).to.contains(data.type);
                expect(inputs[1]).to.contains(data.imageUrl);
                expect(textareas[0]).to.contains(data.description);
                expect(selects[0]).to.contains(data.userType);
            });
            ...
            it("Edit does NOT work with empty fields [ 2.5 Points ]", async function () {
                ...
                let learnMoreInput = await page.waitForSelector('[name="user-type"]', { timeout: interval });

                await typeInput.fill('');
                await imageInput.fill('');
                await descriptionInput.fill('');
                await learnMoreInput.selectOption('');
            ...
            it("Edit sends information to the right API [ 2.5 Points ]", async function () {
                ...
                modifiedData.userType = 'Tattoo Enthusiast';
                ...
                await learnMoreInput.selectOption({ label: modifiedData.userType });
            ...
            it("Edit sends correct information [ 2.5 Points ]", async function () {
                ...
                expect(postData.userType).to.contains(modifiedData.userType);
            });
    ...
    describe('BONUS: Likes [ 15 Points ]', async () => {
        ...
        it("Liking for valid user sends correct API call [ 2.5 Points ]", async function () {
            ...
            const postData = JSON.parse(request.postData());
            expect(postData.tattooId).to.equal(data._id);
            const token = request.headers()["x-authorization"];
            expect(token).to.equal(userData.accessToken, 'Request did not send correct authorization header');
        });
