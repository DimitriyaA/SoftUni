...
import { getSolutionById, deleteSolution } from "../api/api.js";

const detailsTemplate = (solution, onDelete) => html`
<section id="details">
...
export const detailsView = async (ctx) => {
    ...
    const onDelete = async () => {
        ...
        if (confirmed) {
            await deleteSolution(ctx.params.id);
            ctx.page.redirect('/dashboard');
        }
    };

    ctx.render(detailsTemplate(solution, onDelete));
};
